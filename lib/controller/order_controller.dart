...
import '../data/model/response/topu_up_model.dart';
...
class OrderController extends GetxController implements GetxService {
  ...
  PaginatedTopUpModel? _historyFundingModel;
  List<OrderDetailsModel>? _orderDetails;
  List<TopUpModel>? _topUpDetails;
  ...
  PaginatedTopUpModel? get historyFundingModel => _historyFundingModel;
  List<OrderDetailsModel>? get orderDetails => _orderDetails;
  List<TopUpModel>? get topUpDetails => _topUpDetails;
  ...
  Future<ResponseModel?> trackFunding(String? orderID, TopUpModel? topUpModel, bool fromTracking) async {
    _topUpModel = null;
    _responseModel = null;
    if(!fromTracking) {
      _topUpDetails = null;
    ...
    if(topUpModel == null) {
      _isLoading = true;
      Response response = await orderRepo.trackTopUp(orderID);
      if (response.statusCode == 200) {
        _topUpModel = TopUpModel.fromJson(response.body);
        _responseModel = ResponseModel(true, response.body.toString());
      } else {
        _responseModel = ResponseModel(false, response.statusText);
        ApiChecker.checkApi(response);
